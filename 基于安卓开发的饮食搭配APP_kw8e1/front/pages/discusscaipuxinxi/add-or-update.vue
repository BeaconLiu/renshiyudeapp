	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusscaipuxinxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusscaipuxinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusscaipuxinxi`, this.ruleForm);
						}else{
							await this.$api.add(`discusscaipuxinxi`, this.ruleForm);
						}
						uni.setStorageSync('discusscaipuxinxiCleanType',true);
